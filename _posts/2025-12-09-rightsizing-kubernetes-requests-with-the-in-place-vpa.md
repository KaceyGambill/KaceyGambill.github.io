---
layout: post
title: Rightsizing Kubernetes Requests with The In-Place VPA 
date: 2025-12-09 00:00:00 +0000
categories: SRE, Kubernetes, Cost, Efficiency
---
...
1. Evict the Pod.
2. Wait for the Scheduler to recreate it with new numbers.
3. Hope your application handles the graceful shutdown correctly.
...
1. **Audit Mode:** (`updateMode: Off`): We deploy the VPA with updates disabled. We let it run for 1 week to gather metrics on actual usage vs. requests.  
2. **Review Recommendations**: We check the VPA object status (`kubectl describe vpa <name>`) to see what the engine _would_ do. _Pro-tip: If the recommendation is 50% lower than current requests, we know we are wasting money._  
3. **Enable In-Place**: Once we trust the baseline, we switch to `InPlaceOrRecreate`.
4. **Monitor:** We monitor to make sure that we aren’t getting OOMkills and to verify that application metrics are not trending in the wrong direction.

### **Things to Note**

Three things to note before going through this workflow! 
1. Some runtime’s do not currently support the update mode `InPlaceOrRecreate`, it’s dependent on application code or runtimes to support this.
2. When Using an HPA and a VPA on the same resource, be careful that you are not increasing your CPU while the HPA is testing the CPU to see whether or not to scale up additional replicas.
3. Using KEDA to horizontally scale applications based on: **latency, traffic, errors** or **saturation** metrics has essentially solved this for us.
